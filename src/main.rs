...
fn detect_discordant_reads(sam_path: String, genome_path: String, out_prefix: String, anchor_len: i32) {
    ...
    let (code, stdout, stderr) = sh!("bowtie-inspect {}", &genome_path);
    ...
    /*
    let mut test = { Exec::cmd("samtools").args(&["fasta", "-f", "0x4", &sam_path]) | Exec::cmd("fasta").args(&["split", "interleaved", "-", &anchor_len.to_string()])| Exec::cmd("bowtie").args(&["-f", "-p1", "-v0", "-m1", "-B1", "--suppress", "5,6,7,8", &genome_path, "-" ])}.stream_stdout().unwrap();

    let mut alignments = String::new();
    test.read_to_string(&mut alignments);
